...
interface ReviewIncorrectFeedbackProps {
  result: QuestionAttemptResult
  askAiHref: string
}

// 06_ReviewFeedback_Incorrect の本体レイアウト (warn カラー)。
// 構成: ResultBadge:warn → AnswerCard → arrow-down → CorrectCard →
//       ExplanationCard (子要素に CompareBlock + AskAiCta:inline を流し込み)
export function ReviewIncorrectFeedback({
  result,
  askAiHref,
}: ReviewIncorrectFeedbackProps) {
  return (
    <>
