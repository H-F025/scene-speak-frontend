...
interface ReviewProblemScreenProps {
...
// 05_ReviewProblemScreen の Presentational コンポーネント (復習問題専用)。
// 復習ポイントカード / 選択肢 / BottomBar を組み立てる。
//
// Header (タイトル + QuestionCountBadge + 戻る + QuestionProgress) は (main)/layout.tsx の
// 動的 Header (DynamicReviewProblemHeader) が render するため本コンポーネントは保持しない。
// 戻る・タブ移動による finish('abandoned') は ReviewSessionProvider の unmount cleanup で発火
export function ReviewProblemScreen({
  question,
  isSubmitting,
  onSubmit,
}: ReviewProblemScreenProps) {
  const {
    selectedChoiceId,
    isHintShown,
    handleSelectChoice,
